...
-- Reads CARRIED_BALL (set on the carrier by GrabSystem) and stores it as the context
-- target. Returns FAILURE if the entity isn't carrying anything, which aborts the
-- surrounding Serial — i.e. "you can't throw what you aren't holding". Mirror of
...
NodeRegistry.register("SelectCarried", function(_config)
	return {
		...
		execute = function(_self, ctx)
			local ball = world:get(ctx.user, components.CARRIED_BALL)
